...
function Image(elem)
  ...
  -- Create LaTeX for figure based on template type
  local latex_code
  if is_scientific_template(PANDOC_DOCUMENT.meta) then
    latex_code = format_scientific_figure(id, caption, width, path, is_landscape)
  else
    latex_code = format_academic_figure(id, caption, width, path, is_landscape)
  end
...
-- Function to store template information
function Meta(meta)
  -- Store metadata for later use
  PANDOC_DOCUMENT = PANDOC_DOCUMENT or {}
  PANDOC_DOCUMENT.meta = meta
  return nil
end
